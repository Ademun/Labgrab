...
CREATE INDEX idx_subscriptions_lookup ON subscription_service.subscriptions (
                                                                             lab_type,
                                                                             lab_topic,
                                                                             lab_number,
                                                                             status
...
(
    ...
    user_uuid   uuid        not null,
    constraint time_preferences_pk primary key (
                                                week_number,
                                                day_of_week,
                                                user_uuid
        )
...
CREATE INDEX idx_teacher_prefs_user ON subscription_service.teacher_preferences (user_uuid) INCLUDE (blacklisted_teachers);
...
CREATE INDEX idx_details_user_stats ON subscription_service.details (user_uuid) INCLUDE (
